...
class ATTextField: UITextField {
    ...
    @IBInspectable dynamic open var placeholderColor: UIColor = ATTextField.Constants.defaultiOSPlaceholderColor {
        didSet {
            updatePlaceholder()
        }
    }
    ...
    /// A UIColor value that determines text color of the placeholder label
    @IBInspectable dynamic open var borderViewColor: UIColor = .white {
        didSet {
    ...
    func setBottomBorder() {
        if bottomBorder {
            if borderView.isDescendant(of: self) {
                return
            }
           ...
           didSet {
            ...
            if let titleColorData = UserDefaults.standard.object(forKey: DefaultKeys.titlecolorKey) as? Data{
                           do {
                            ...
                            layer.borderColor = (titleColor ?? .black).cgColor
...
protocol InputviewTextFieldDelegate {
    func textFieldDidDelete(_ textField: UITextField)
}

class InpuviewTextField: ATTextField {
    var inputviewDelegate: InputviewTextFieldDelegate?
    
    override func deleteBackward() {
        super.deleteBackward()
        inputviewDelegate?.textFieldDidDelete(self)
    }
...
protocol ToolbarPickerViewDelegate: class {
    func didTapDone(with pickerView: UIPickerView)
}
...
class ToolbarPickerView: UIPickerView {
    ...
    @objc func doneTapped() {
        self.toolbarDelegate?.didTapDone(with: self)
    }
}

protocol DateToolbarPickerViewDelegate: class {
    func didTapDone(with datePicker: UIDatePicker)
}

class DateToolbarPickerView: UIDatePicker {
    
    public private(set) var toolbar: UIToolbar?
    public weak var toolbarDelegate: DateToolbarPickerViewDelegate?
...
@IBDesignable
open class ATFloatingTextField: UITextField { // swiftlint:disable:this type_body_length
    private struct Constants {
        static let defaultiOSPlaceholderColor = UIColor(red: 0.0, green: 0.0, blue: 0.0980392, alpha: 0.22)
    }
    ...
    /// A UIColor value that determines text color of the placeholder label
    @IBInspectable dynamic open var placeholderColor: UIColor = ATFloatingTextField.Constants.defaultiOSPlaceholderColor {
        didSet {
            updatePlaceholder()
        }
    }
    ...
    @IBInspectable dynamic open var placeholderFont: UIFont = UIFont.systemFont(ofSize: 18) {
        didSet {
            updatePlaceholder()
        }
    }
...
//            string: placeholder,
//            attributes: [
//                NSAttributedString.Key.foregroundColor: color, NSAttributedString.Key.font: placeholderFont
//            ]
//        )
        if let titleColorData = UserDefaults.standard.object(forKey: DefaultKeys.titlecolorKey) as? Data{
                       do {
                           let titleColor = try NSKeyedUnarchiver.unarchivedObject(ofClass: UIColor.self, from: titleColorData)
                               ...
                               attributedPlaceholder = NSAttributedString(
                                   string: placeholder,
                                   attributes: [
                                    NSAttributedString.Key.foregroundColor: titleColor ?? .black, NSAttributedString.Key.font: placeholderFont
                                   ]
                               )
    ...
    @objc dynamic open var titleFont: UIFont = UIFont.systemFont(ofSize: 12)
        /*{
        didSet {
            updateTitleLabel()
        }
    ...
    fileprivate final func init_MPFloatingTextField() {
        borderStyle = .none
        createTitleLabel()
        updateColors()
    ...
    fileprivate func addEditingChangedObserver() {
        self.addTarget(self, action: #selector(ATFloatingTextField.editingChanged), for: .editingChanged)
    }
    ...
    override open func textRect(forBounds bounds: CGRect) -> CGRect {
        ...
        let rect = CGRect(
            ...
            height: superRect.size.height - titleHeight - 15
        )
        return rect
    }
    ...
    override open func placeholderRect(forBounds bounds: CGRect) -> CGRect {
        let rect = CGRect(
            ...
            height: bounds.size.height - titleHeight() - 15
        )
        return rect
    }
    ...
    /// A UIColor value that determines superview border color change
    @IBInspectable dynamic open var isHighlightOnRespond: Bool = false
    @IBInspectable dynamic open var responderColor: UIColor = .white
    @IBInspectable dynamic open var responderborderWidth: CGFloat = 0
    @IBInspectable dynamic open var defaultImage: UIImage?
    @IBInspectable dynamic open var selectedImage: UIImage?
    @IBInspectable dynamic open var errorImage: UIImage?
...
//        )
        ...
        do {
            let titleColor = try NSKeyedUnarchiver.unarchivedObject(ofClass: UIColor.self, from: titleColorData)
                ...
                attributedPlaceholder = NSAttributedString(
                                   string: placeholder,
                                   attributes: [
                                       NSAttributedString.Key.foregroundColor: titleColor, NSAttributedString.Key.font: font
                                   ]
                               )
    ...
    override open func textRect(forBounds bounds: CGRect) -> CGRect {
        ...
        return CGRect(x: superRect.origin.x + paddingLeft,
                      y: titleHeight,
                      width: superRect.width - paddingLeft - paddingRight,
